...
def api_process_response(request):
    """Process user response with expanded conversation flow"""
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            user_message = data.get('message', '').lower().strip()
            current_step = data.get('current_step', 1)
            conversation_state = data.get('conversation_state', 'asking_name')
            user_data = data.get('user_data', {})
            
            response = process_conversation(user_message, current_step, conversation_state, user_data)
            return JsonResponse(response)
            
        except Exception as e:
            return JsonResponse({
                'step_id': 1,
                'avatar_message': "Let's start over! What's your name?",
                'conversation_state': 'asking_name',
                'user_data': {}
            })
